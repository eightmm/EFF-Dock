#!/usr/bin/env python3
"""Bind one standalone confidence-selector PoseBusters shard to sampling inputs."""
...
BINDING_CONTRACT = LEGACY_V1.binding_contract
...
def _file_ledger(
    ...
    *,
    ...
    digest = hashlib.sha256()
    digest.update(b"EFFDOCK_ETA_SWEEP_CONFIDENCE_STANDALONE_INPUT_FILE_LEDGER_V1\0")
    for row in rows:
...
def build_binding(
    *,
    ...
    if protocol_id != spec.protocol_id:
        raise ValueError(f"protocol_id must be {spec.protocol_id!r}")
    if selector not in SELECTORS:
        raise ValueError(f"selector must be one of {SELECTORS}")
    if not 0 <= shard_index < num_shards:
        raise ValueError("shard_index must satisfy 0 <= shard_index < num_shards")

    integrity_binding: dict[str, Any] | None = None
    if spec == STERIC_HIGH_ETA_V1:
        if integrity_audit is None or not Path(integrity_audit).is_file():
            raise FileNotFoundError("high-eta V2 binding requires the passed full integrity audit")
        integrity_path = Path(integrity_audit).resolve()
        ...
        if not isinstance(integrity, dict) or any(
            integrity.get(key) != value for key, value in expected_integrity.items()
        ):
            raise ValueError("high-eta V2 binding integrity-audit identity mismatch")
        diagnostic = validate_v2_prior_pool_sha256_diagnostics(integrity, spec=spec)
        integrity_binding = {
            "path": str(integrity_path),
            "sha256": file_sha256(integrity_path),
            "schema_version": spec.audit_schema_version,
            "audit_contract": spec.audit_contract,
            "global_integrity_ledger_sha256": _require_sha256(
                integrity.get("global_integrity_ledger_sha256"),
                label="integrity_audit.global_integrity_ledger_sha256",
            ),
            "sampling_seed_pairing": "exact_frozen_sorted_id_offset",
            "prior_pool_sha256_cross_eta_identity_claim": False,
            "prior_pool_sha256_policy": diagnostic["policy"],
            "complexes_with_multiple_prior_pool_sha256": diagnostic[
    ...
    sampling_tag = run_name if num_shards == 1 else f"{run_name}.{tag}"
    sampling_summary = sampling_dir / f"{sampling_tag}.summary.json"
    sampling_csv = sampling_dir / f"{sampling_tag}.csv"
    ...
    return {
        "binding_contract": spec.binding_contract,
        "protocol_id": spec.protocol_id,
        "run_name": run_name,
        "dataset": dataset,
        "eta": eta,
        ...
        **({"integrity_audit": integrity_binding} if integrity_binding is not None else {}),
    }
...
def main(argv: list[str] | None = None) -> None:
    ...
    binding = build_binding(
        ...
        integrity_audit=args.integrity_audit,
