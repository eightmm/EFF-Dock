...
INLINE_SIZE_LIMIT_BYTES = 2_000_000
...
INTERACTION_TERM_LABELS = {
    "interaction_hydrophobic": "hydrophobic",
    "interaction_hydrogen_bond": "H-bond",
    "interaction_screened_formal_charge": "charge",
    "interaction_pi_stacking": "π-stack",
    "interaction_cation_pi": "cation–π",
    "interaction_halogen_bond": "halogen",
    "interaction_metal_coordination": "metal",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Write a dependency-free inline HTML fragment. Unless "
            "--render-static is also given, this writes no GIF or PNG."
        ),
    )
    ...
    output_mode.add_argument(
        ...
        action="store_true",
        help=(
            "Require --inline-html and explicitly suppress GIF/PNG output "
            "(already the default when --inline-html is given)."
        ),
    )
    output_mode.add_argument(
        "--render-static",
        action="store_true",
        help="Render relaxation.gif and dashboard.png, including alongside --inline-html.",
    )
...
def _canonical_mode(key: str, *metadata: Mapping[str, Any]) -> str | None:
    candidates: list[Any] = []
    for source in metadata:
        candidates.extend(
            source.get(name)
            for name in ("mode", "guidance_mode", "interaction_mode")
            if source.get(name) is not None
        )
    candidates.append(key)
    for candidate in candidates:
        normalised = _normalised_name(candidate)
        if normalised in {"unified", "full", "guidance"}:
            return "unified"
        if normalised in {"physical", "physical_only"}:
            return "physical"
    return None


def _humanize_run_name(value: Any) -> str:
    words = " ".join(
        str(value).strip().replace("_", " ").replace("-", " ").replace("/", " / ").split()
    )
    return words[0].upper() + words[1:] if words else "Unnamed run"


def _explicit_label(*metadata: Mapping[str, Any]) -> str | None:
    for source in metadata:
        for name in ("label", "display_label", "display_name", "name"):
...
def _run_label(
    ...
    label_maps = []
    for source in (bundle, summary):
        labels = source.get("run_labels")
        if isinstance(labels, Mapping) and key in labels:
            label_maps.append({"label": labels[key]})
    explicit = _explicit_label(run_bundle, run_summary, *label_maps)
    if explicit is not None:
        return explicit

    mode = _canonical_mode(key, run_bundle, run_summary)
    detail_parts: list[str] = []
    for source in (run_bundle, run_summary):
        ensemble = source.get("ensemble_name", source.get("ensemble"))
        prior = source.get(
            "prior_label",
            source.get("prior", source.get("prior_mode", source.get("initialization_mode"))),
        )
        seed = source.get("seed", source.get("prior_seed"))
        if ensemble is not None and str(ensemble).strip():
            detail_parts.append(_humanize_run_name(ensemble))
        if prior is not None and str(prior).strip():
            detail_parts.append(_humanize_run_name(prior))
    ...
    if detail_parts:
        if mode == "unified":
            detail_parts.append("Unified guidance")
        elif mode == "physical":
            detail_parts.append("Physical only")
        deduplicated = list(dict.fromkeys(part.casefold() for part in detail_parts))
        return " · ".join(
            next(part for part in detail_parts if part.casefold() == folded)
            for folded in deduplicated
        )
    if mode == "unified":
        return "Unified guidance"
    if mode == "physical":
        return "Physical only"
    return _humanize_run_name(key)
...
def _humanize_initialization(value: Any) -> str | None:
    ...
    if {"paired", "local", "model"} <= tokens:
        return "Paired local + model priors"
    if "tear" in lowered:
    ...
    if "model" in tokens and "prior" in tokens:
        return "Model fragment prior"
...
def _interaction_energy_names(metrics: Any) -> list[str]:
    sources: list[Mapping[str, Any]] = []
    if isinstance(metrics, Mapping):
        rows = _mapping_rows(metrics)
        sources = rows if rows is not None else [metrics]
    elif isinstance(metrics, Sequence) and not isinstance(metrics, (str, bytes)):
        sources = [row for row in metrics if isinstance(row, Mapping)]

    names: set[str] = set()
    for source in sources:
        energy_mappings = [source.get(group_name) for group_name in ("energies", "energy")]
        energy_mappings.append(source)
        for energy_mapping in energy_mappings:
            if not isinstance(energy_mapping, Mapping):
                continue
            names.update(
                name
                for name in energy_mapping
                if isinstance(name, str) and name.startswith("interaction_")
            )
    preferred = {name: index for index, name in enumerate(INTERACTION_TERM_ORDER)}
    return sorted(names, key=lambda name: (preferred.get(name, len(preferred)), name))


def _interaction_term_label(name: str) -> str:
    if name in INTERACTION_TERM_LABELS:
        return INTERACTION_TERM_LABELS[name]
    return _humanize_run_name(name.removeprefix("interaction_"))
...
def _load_run(
    ...
    if isinstance(summary_runs, Mapping):
        ...
        mode = _canonical_mode(key, run_bundle)
        if summary_key is None:
            aliases = {
                "physical": ("physical", "physical_only"),
                "unified": ("unified", "full", "guidance"),
            }.get(mode)
    ...
    mode = _canonical_mode(key, run_bundle, run_summary)
    label = _run_label(key, run_bundle, run_summary, bundle, summary)
    ...
    for term_name in _interaction_energy_names(metrics):
        extracted[term_name] = _align_series(
            _raw_metric_series(
                metrics,
                (
                    ("energies", term_name),
...
def load_render_data(summary_path: Path, trajectory_path: Path) -> RenderData:
    ...
    bundle_runs = bundle["runs"]
    if not bundle_runs:
        raise ValueError("trajectory bundle runs mapping must not be empty")
    runs: dict[str, RunData] = {}
    for raw_key in bundle_runs:
        if not isinstance(raw_key, str) or not raw_key.strip():
            raise ValueError("trajectory run keys must be non-empty strings")
        runs[raw_key] = _load_run(
            raw_key,
            bundle,
            summary,
            crystal,
...
def render_gif(
    ...
    *,
    ...
    run = data.runs[_pick_key(data.runs, ("unified", "full", "guidance"))]
...
def render_dashboard(data: RenderData, camera: Camera, path: Path) -> None:
    ...
    physical_key = _pick_key(data.runs, ("physical", "physical_only"))
    unified_key = _pick_key(data.runs, ("unified", "full", "guidance"))
    scene_boxes = {
        physical_key: (50, 105, 775, 555),
        unified_key: (825, 105, 1550, 555),
    }
    ...
    physical = data.runs[physical_key]
    unified = data.runs[unified_key]
    ...
    *,
    ...
    return {
        ...
        "interactionTerms": [
            {
                "key": name,
                "label": _interaction_term_label(name),
                "values": [_metric_at(run, name, index) for index in indices],
            }
            for name in _interaction_energy_names(run.metrics)
        ],
    }
...
def write_inline_html(data: RenderData, camera: Camera, path: Path) -> None:
    ...
    while True:
        ...
        if len(template.encode()) + len(encoded.encode()) < INLINE_SIZE_LIMIT_BYTES:
            break
        if maximum_frames <= 2:
            raise ValueError("inline visualization exceeds 2 MB even after trajectory downsampling")
        maximum_frames = max(2, maximum_frames // 2)
    root_suffix = "".join(
    ...
    if len(fragment.encode()) >= INLINE_SIZE_LIMIT_BYTES:
        raise ValueError("inline visualization exceeds the 2 MB size limit")
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    args = parse_args()
    if args.inline_only and args.inline_html is None:
        raise SystemExit("--inline-only requires --inline-html")
    render_static = args.render_static or args.inline_html is None
    if render_static and args.max_gif_frames < 2:
