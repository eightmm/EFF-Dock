...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--receptor-column", default="predicted_receptor")
...
def configure_surface_imports(surfdock_root: Path) -> Any:
    ...
    # SurfDock first crops an 8 A residue pocket and then retains only surface
    # faces whose every vertex is within 3 A of the ligand.  For a few deeply
    # buried ligands that second set is empty, and PyMesh fails while repairing
    # a zero-face mesh.  The full molecular surface is still that of the local
    # 8 A pocket, so retain it as a bounded compatibility fallback.
    upstream_submesh = computeTargetMesh_test_samples.pymesh.submesh
    upstream_fix_mesh = computeTargetMesh_test_samples.fix_mesh
    submesh_state = {"empty_face_fallbacks": 0, "last_full_mesh": None}

    def nonempty_pocket_submesh(mesh: Any, elements: Any, num_rings: int) -> Any:
        submesh_state["last_full_mesh"] = mesh
        if len(elements) == 0:
            submesh_state["empty_face_fallbacks"] += 1
            print(
                "[SurfDock compatibility] the native 3A surface-face crop was empty; "
                "retaining the complete supplied 8A-pocket surface.",
                flush=True,
            )
            return mesh
        cropped = upstream_submesh(mesh, elements, num_rings)
        if cropped.num_faces == 0 and mesh.num_faces > 0:
            submesh_state["empty_face_fallbacks"] += 1
            print(
                "[SurfDock compatibility] PyMesh reduced the non-empty native "
                "3A face selection to zero faces; retaining the complete "
                "supplied 8A-pocket surface.",
                flush=True,
            )
            return mesh
        return cropped

    def robust_pocket_fix_mesh(mesh: Any, resolution: float) -> Any:
        try:
            return upstream_fix_mesh(mesh, resolution)
        except ValueError as error:
            full_mesh = submesh_state.get("last_full_mesh")
            if (
                "zero-size array" not in str(error)
                or full_mesh is None
                or full_mesh.num_faces == 0
            ):
                raise
            submesh_state["empty_face_fallbacks"] += 1
            print(
                "[SurfDock compatibility] the native cropped surface could not "
                "be regularized; retrying mesh repair on the complete supplied "
                "8A-pocket surface.",
                flush=True,
            )
            return upstream_fix_mesh(full_mesh, resolution)

    computeTargetMesh_test_samples.pymesh.submesh = nonempty_pocket_submesh
    computeTargetMesh_test_samples.fix_mesh = robust_pocket_fix_mesh
    computeTargetMesh_test_samples._effdock_submesh_state = submesh_state
...
def main() -> None:
    ...
    required_columns = {"complex_name", "reference_ligand", args.receptor_column}
    ...
    for row in rows:
        ...
        safe_link(Path(row[args.receptor_column]), protein)
    ...
    for row in rows:
        ...
        error = None
        fallback_count_before = surface_module._effdock_submesh_state[
            "empty_face_fallbacks"
        ]
        ...
        surface_status[target_id] = {
            ...
            "empty_3A_face_crop_fallback": (
                surface_module._effdock_submesh_state["empty_face_fallbacks"]
