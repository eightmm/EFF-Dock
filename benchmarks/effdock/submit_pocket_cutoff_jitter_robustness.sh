...
output_root="$repo_root/outputs/benchmarks/effdock_pocket_cutoff_jitter_robustness_runs/$run_id"
execution_root="$repo_root/.effdock_execution_capsules/$protocol_id/$run_id"
jitter0_root="$repo_root/outputs/benchmarks/effdock_pocket_cutoff_robustness_runs/cutoff-r3-production-20260901-r2"
...
"$repo_root/.venv/bin/python" - "$jitter0_root/report.json" <<'PY'
import json
import pathlib
import sys
...
assert payload["protocol_id"] == "EFFDOCK-POCKET-CUTOFF-ROBUSTNESS-V1"
assert payload["total_selected_posebusters_errors"] == 0
assert set(payload["datasets"]) == {"astex", "posebusters"}
assert all(len(payload["datasets"][name]["cutoffs"]) == 4 for name in payload["datasets"])
PY
...
copy_files=(
  ...
  benchmarks/effdock/report_pocket_cutoff_jitter_robustness.py
  benchmarks/effdock/slurm/pocket_cutoff_jitter_end_to_end_smoke.sbatch
  benchmarks/effdock/slurm/pocket_cutoff_jitter_generation.sbatch
  benchmarks/effdock/slurm/pocket_cutoff_jitter_manifest.sbatch
  benchmarks/effdock/slurm/pocket_cutoff_jitter_refine_confidence.sbatch
  benchmarks/effdock/slurm/pocket_cutoff_jitter_selected_posebusters.sbatch
  benchmarks/effdock/slurm/pocket_cutoff_jitter_report.sbatch
...
protocol_sha=$(sha256sum docs/EFFDOCK_POCKET_CUTOFF_JITTER_ROBUSTNESS_PROTOCOL.md | cut -d' ' -f1)
baseline_sha=$(sha256sum "$jitter0_root/report.json" | cut -d' ' -f1)
...
printf '%s\n' \
  ...
  'docking_cutoffs_angstrom=6,8,10,12' 'center_jitter_sigma_per_axis=0,1,2' \
  'newly_computed_jitter_sigma_per_axis=1,2' 'repeats=3' \
  ...
  "jitter0_root=$jitter0_root" "jitter0_report_sha256=$baseline_sha" \
...
base_export="ALL,EFFDOCK_REPO_DIR=$execution_root_abs,EFFDOCK_RUNTIME_VENV=$repo_root/.venv,PYTHONPATH=$execution_root_abs/src,PYTHONDONTWRITEBYTECODE=1,EFFDOCK_OUTPUT_ROOT=$output_root,EFFDOCK_JITTER0_ROOT=$jitter0_root"
submitted=()
committed=0
cleanup() {
  code=$?
...
smoke_job=$(submit '' '6000ada,heavy' '0-1%2' "$base_export" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_end_to_end_smoke.sbatch); submitted+=("$smoke_job")
generation_job=$(submit "$smoke_job" '6000ada,heavy' '0-383%12' "$base_export" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_generation.sbatch); submitted+=("$generation_job")
manifest_job=$(submit "$generation_job" cpu_only '0-23%12' "$base_export" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_manifest.sbatch); submitted+=("$manifest_job")
refinement_job=$(submit "$manifest_job" '6000ada,heavy' '0-767%12' \
  "$base_export,EFFDOCK_STAGE=refinement" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_refine_confidence.sbatch); submitted+=("$refinement_job")
confidence_job=$(submit "$refinement_job" '6000ada,heavy' '0-767%12' \
  "$base_export,EFFDOCK_STAGE=confidence" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_refine_confidence.sbatch); submitted+=("$confidence_job")
posebusters_job=$(submit "$confidence_job" cpu_only '0-383%12' "$base_export" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_selected_posebusters.sbatch); submitted+=("$posebusters_job")
report_job=$(submit "$posebusters_job" cpu_only '' "$base_export" \
  benchmarks/effdock/slurm/pocket_cutoff_jitter_report.sbatch); submitted+=("$report_job")
