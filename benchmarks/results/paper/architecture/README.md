...
detail, normalization/activation and scalar MLP internals. The overview shows the atom-head sum, gated ligand-state input
...
labels identify B/C interfaces; degree classes use ℓ = 0 and ℓ > 0. E expands
the radial, gate/norm and confidence MLPs; B brackets the deployed equivariant
block. The unused reusable EquivariantMLP class is not a model stage.
