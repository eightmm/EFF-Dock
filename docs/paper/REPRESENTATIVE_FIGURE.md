...
read left to right across the top row, then across the bottom row, joined by a
continuous bent arrow. Each stage has a short method subtitle directly under
its heading. Confidence displays three distinct recorded poses with selection
marks; all pose panels have equal dimensions.
...
| Confidence candidates | Astex repeat 0, same 1T46 complex, existing N100 refined bank; eligible ranks 1, 50 and 100 |
...
chosen before inspecting their structures, using best, lower-median and worst
predicted-RMSD ranks (1, 50, 100; zero-based bank indices 41, 3, 84). The best is
...
RMSD correctness or a probability. The two unselected structures are distinct
candidates, not intermediate states of the selected trajectory. The figure does
not imply that they were generated by the displayed N1 run. These three
...
the three displayed coordinates and their source provenance. Endpoint backing
cards remain conceptual multiplicity icons. The primary selector ranks eligible
...
with a common 0.70 zoom factor so that all three confidence candidates are fully
...
Generation times and refinement iterations are labeled separately. The endpoint backing cards indicate candidate
...
25, 50 and 100. Each sequence follows the connected arrows across the top row
and then the bottom row; refinement step 0 equals the generated t = 1 pose.
Method subtitles appear beneath the stage headings; η-based guidance is omitted.
The confidence stage displays three distinct refined candidates from the existing
N100 bank for the same 1T46–STI complex, at eligible predicted-RMSD ranks 1, 50
and 100. The best-ranked candidate is marked ✓ and shown as the selected output;
× marks the two unselected candidates, not physical invalidity. All molecular
...
mapping and no geometric alignment, new scoring or inference. Endpoint backing
cards denote multiplicity schematically. The refinement example's finite-shell
