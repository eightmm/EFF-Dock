...
within $5\,\mathrm{\AA}$ condition six equivariant interaction layers, using
...
Main sampling uses $N=100$ candidates, $S=10$ steps, $\sigma=2\,\mathrm{\AA}$
and a $10\,\mathrm{\AA}$ crop. The late-power-three Euler grid and updates are
\begin{align}
...
strict $2\,\mathrm{\AA}$ labels. Let $y_k=\log(1+r_k)$:
\begin{align}
...
$E+10^{-10}\max(1,|E|)$. Each step is capped at $0.10\,\mathrm{\AA}$
translation, five degrees rotation and $0.10\,\mathrm{\AA}$ actual atom
movement. At most 100 iterations are run. Stop after 20 consecutive accepted
steps with maximum atom movement below $10^{-5}\,\mathrm{\AA}$, or, from
iteration 25, five consecutive steps with energy decrease at most
$0.02+0.001\max(1,|E|)$. External runs use an $18\,\mathrm{\AA}$ receptor
shell, $8\,\mathrm{\AA}$ physical cutoff and $10\,\mathrm{\AA}$ maximum
interaction cutoff. Training-bank refinement instead used displacement
threshold $0.01\,\mathrm{\AA}$ with patience five. Failed line searches retain
...
LJ softcore decreases from 1.5 to $0.75\,\mathrm{\AA}$ over the active
...
$0.25\,\mathrm{\AA}$. The sampler adds the drift to the learned field before
...
\begin{equation}
 \mathrm{SR}=\frac{100}{M}\sum_i\mathbf1[r_{i,k_i^*}<2\,\mathrm{\AA}],\quad
 \mathrm{SR}_{\rm PB}=\frac{100}{M}\sum_i\mathbf1[r_{i,k_i^*}<2\,\mathrm{\AA}]b_{i,k_i^*}.
\end{equation}
