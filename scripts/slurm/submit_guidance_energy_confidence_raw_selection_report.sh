...
for required in \
  .venv/bin/python \
  docs/GUIDANCE_ENERGY_CONFIDENCE_RAW_SELECTION_PROTOCOL.md \
  scripts/report_guidance_energy_confidence_raw_selection.py \
...
copy_files=(
  ...
  scripts/slurm/guidance_energy_confidence_raw_selection_report.sbatch
  scripts/slurm/submit_guidance_energy_confidence_raw_selection_report.sh
...
raw=$(sbatch --parsable --partition=cpu_only --export="$base_export" \
  "$execution_root_abs/scripts/slurm/guidance_energy_confidence_raw_selection_report.sbatch")
job_id=${raw%%;*}
...
printf '%s\n' \
  ...
  > "$input_root/.raw_energy_confidence_selection_submission.$run_id"
printf 'report_job=%s\noutput_root=%s\nexecution_root=%s\n' \
