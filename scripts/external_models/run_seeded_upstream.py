...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Bypass DiffBindFR's eager PyMOL import for the native "
            "--no_error_correction path. The shim raises if PyMOL is used."
        ),
    )
    ...
    if args.stub_diffbindfr_pymol:
        ...
        class UnavailablePyMOL:
            def __init__(self, *_args: object, **_kwargs: object) -> None:
                raise RuntimeError(
                    "PyMOL is unavailable in the DiffBindFR "
                    "--no_error_correction benchmark arm"
                )
